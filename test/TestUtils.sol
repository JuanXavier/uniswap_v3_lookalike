...
/**
 * In our smart contract tests, we need a way to round ticks and convert rounded prices to √P
 * In a previous chapter, we chose to use ABDKMath64x64 to handle fixed-point numbers math in tests.
 * The library, however, doesn’t implement the rounding function nearestUsableTick, so we’ll need to implement it:
 */
abstract contract TestUtils is Test, Assertions {
    ...
    // 2. it then rounds the result to the decimal one (result = quot >> 64), the fractional part is lost at this point
    // (i.e. the result is rounded down);
    // 3. it then divides the quotient by 2 ** 64
    //  , takes the remainder, and compares it with 0x8000000000000000 (which is 0.5 in Q64.64);
