  ...
  Promise.all([token0.allowance(account, managerAddress), token1.allowance(account, managerAddress)])
    .then(([allowance0, allowance1]) => {
      return Promise.resolve()
        .then(() => {
          if (allowance0.lt(amount0)) return token0.approve(managerAddress, amount0).then((tx) => tx.wait());
        })
        .then(() => {
          if (allowance1.lt(amount1)) return token1.approve(managerAddress, amount1).then((tx) => tx.wait());
        })
        .then(() => {
...
const swap = (amountIn, account, { tokenIn, manager, token0, token1 }, { managerAddress, poolAddress }) => {
  ...
  tokenIn
    ...
    .then((allowance) => {
      if (allowance.lt(amountInWei)) return tokenIn.approve(managerAddress, amountInWei).then((tx) => tx.wait());
    })
    .then(() => {
