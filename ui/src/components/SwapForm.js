...
const addLiquidity = (account, { token0, token1, manager }) => {
  ...
  Promise.all([token0.allowance(account, config.managerAddress), token1.allowance(account, config.managerAddress)])
    .then(([allowance0, allowance1]) => {
      return Promise.resolve()
        ...
        .then(() => {
          return manager.mint(config.poolAddress, lowerTick, upperTick, liquidity, extra).then((tx) => tx.wait())
        })
        .then(() => {
...
const swap = (zeroForOne, amountIn, account, { tokenIn, manager, token0, token1 }) => {
  ...
  tokenIn
    ...
    .then((allowance) => {
      if (allowance.lt(amountInWei)) return tokenIn.approve(config.managerAddress, uint256Max).then((tx) => tx.wait())
    })
    .then(() => {
